...
at **error** level across all of `src/` — every layer has had its pass.
`max-lines-per-function` is off in two places where it measures the wrong thing:
under `src/state/` it reads a slice factory — an object literal of actions — as
one enormous function, and in `.tsx` files it counts a component's JSX tree.
Both keep the per-function limit by review; hooks and helpers in `.ts` files are
still checked. Run `make lint`.
...
No file is awaiting a pass: the frontend is fully within the limits. The
recurring shapes these passes settled on, for the next one:

- A large component becomes a `useXModel()` hook holding its state, derived
  values and handlers, plus band components that read `{ model: m }`.
- A view's tabs and panels each become their own component, in a folder named
  after the view (`ui/budget/`, `ui/views/today/`).
- Flat conditional maps (DTO mappers, class-name lists, defaulting tables) keep
  an inline `complexity` disable with the reason, rather than being split into
  helpers that only add indirection — rule 5.
